...
public class SGWorldSkyRenderer extends IRenderHandler {
	...
	public SGWorldSkyRenderer(IWorldData wdata) {
		worldData = wdata;
	}
	...
	private void renderType(FeatureType type, float partialTicks, WorldClient world, Minecraft mc) {
		for (WorldFeature feat : this.worldData.getFeatures(type)) {
			if (feat instanceof WorldFeature.WorldFeatureRender) {
				GL11.glPushMatrix();
				GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
				((WorldFeature.WorldFeatureRender)feat).render(partialTicks, world, mc);
				GL11.glPopMatrix();
			}
		}
	}
